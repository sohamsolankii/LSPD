...
//   }, []);
  ...
  const handleAddNews = async () => {
    try {
      ...
      if (isEditing) {
        const response = await axios.put(`/api/news/${currentNews.id}`, formData, {
          headers: {
            'Content-Type': 'multipart/form-data',
          },
        });
        setNews(news.map((newsItem) => (newsItem.id === currentNews.id ? response.data : newsItem)));
        setIsEditing(false);
        setCurrentNews(null);
      } else {
  ...
  return (
    <div className="p-4 md:p-6 poppins dark:bg-gray-100 bg-[var(--bg2)]">
      <div className="mb-6 rounded-2xl shadow-black/70 dark:shadow-black/10 bg-[var(--bg1)] dark:bg-gray-100 dark:border-gray-400 border-[1px] border-[var(--opac)] shadow-2xl">
        ...
        <form className="grid grid-cols-1 gap-5 md:grid-cols-2 p-5" onSubmit={handleSubmit(handleAddNews)}>
          <div className="flex flex-col">
            <label className="text-sm text-gray-500 pb-2">Upload Image</label>
            <input
              type="file"
              accept="image/*"
              {...register('image')}
            ...
            />
            {newNews.image && (
              <img src={URL.createObjectURL(newNews.image)} alt="Uploaded" className="mt-2 h-40 object-contain" />
            )}
          </div>
          <div className="flex flex-col">
            <label className="text-sm text-gray-500 pb-2">Enter the news title</label>
            <input
              type="text"
          ...
          <div className="flex flex-col col-span-2">
            <label className="text-sm text-gray-500 pb-2">Provide the news description</label>
            <textarea
          ...
          <div className="md:col-span-2 flex justify-center">
            ...
            >
              {isEditing ? 'Update News' : 'Add News'} <FaPlus className="ml-2" />
            </button>
          </div>
        </form>
      </div>
      ...
      <div className="mb-6 dark:bg-gray-100 dark:border-gray-400 rounded-2xl shadow-black/70 dark:shadow-black/10 bg-[var(--bg1)] border-[1px] border-[var(--opac)] shadow-2xl">
        <h2 className="text-2xl rounded-xl z-10 font-medium dark:bg-gray-100 dark:border-gray-400 m-2 p-2 text-[var(--lgold)] dark:text-[var(--dltext)] text-center shadow-black/70 dark:shadow-black/10 bg-[var(--bg1)] border-[1px] border-[var(--opac)] shadow-2xl">Current News</h2>
        <table className="min-w-full mb-4 rounded-md shadow-black/30 text-sm md:text-lg text-gray-200">
          <thead>
            <tr>
              <th className="py-2 font-medium text-gray-500">News Title</th>
          ...
          <tbody>
            {news.map((newsItem) => (
              <tr key={newsItem.id}>
                <td className="border-[1px] border-[var(--opac)] dark:border-gray-300 font-regular text-gray-300 dark:text-gray-500 px-4 py-2">{newsItem.title}</td>
                <td className="border-[1px] border-[var(--opac)] dark:border-gray-300 font-regular text-gray-300 dark:text-gray-500 px-4 py-2">
                  <button
                    onClick={() => handleShowDetails(newsItem)}
      ...
      {showDetails && currentNews && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white p-6 rounded-lg shadow-lg w-11/12 md:w-2/3 lg:w-1/2">
            <h3 className="text-2xl font-bold mb-4">{currentNews.title}</h3>
            {currentNews.image && (
              <img src={currentNews.image} alt={currentNews.title} className="mb-4" />
            )}
